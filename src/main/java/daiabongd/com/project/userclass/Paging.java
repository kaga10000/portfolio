...
public class Paging {
	...
	public Paging(int maxNum, int pageNum, int listCount, int pageCount, String refineUrl) {
		this.maxNum = maxNum;
	...
	public String makeHtmlPaging() {
		...
		return makeHtml(currentGroup, totalPage, refineUrl);
	}

	private String makeHtml(int currentGroup, int totalPage, String refineUrl) {
		...
		if (start != 1) {
			sb.append("<a href='"+refineUrl+"?pageNum=" + (start -1) + "'>");
			sb.append("[이전]");
			sb.append("</a>");
		}
		...
		for (int i = start; i <= end; i++) {
			if (pageNum != i) { //현재 페이지가 아닌 경우 링크처리
				sb.append("<a id='nextpage' href='"+refineUrl+"?pageNum=" + i + "'>");
				sb.append(" [ ");
				sb.append(i);
				sb.append(" ] ");
				sb.append("</a>");
		...
		if (end != totalPage) {
			sb.append("<a href='"+refineUrl+"?pageNum=" + (end + 1) + "'>");
			sb.append("[다음]");
			sb.append("</a>");
		}
		return sb.toString();
