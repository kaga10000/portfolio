...
public class Product_Paging {
	private int maxNum; 		// 전체 글의 개수
	private int pageNum; 		// 현재 페이지 번호
	private int listCount; //12		// 페이지당 나타낼 글의 갯수
	private int pageCount; //2		// 페이지그룹당 페이지 갯수
	private String refineUrl; 	// 게시판 이름
	private String Sort; 	// 상품 분류

	public Product_Paging(int maxNum, int pageNum, int listCount, int pageCount, String refineUrl,String Sort) {
		this.maxNum = maxNum;
	...
	public String makeHtmlPaging() {
		...
		return makeHtml(currentGroup, totalPage, refineUrl ,Sort);
	}

	private String makeHtml(int currentGroup, int totalPage, String refineUrl , String Sort) {
		...
		if (start != 1) {
			sb.append("<a href='"+refineUrl+"?Sort="+Sort+"&pageNum=" + (start -1) + "'>");
			sb.append("[이전]");
			sb.append("</a>");
		}
		...
		for (int i = start; i <= end; i++) {
			if (pageNum != i) { //현재 페이지가 아닌 경우 링크처리
				sb.append("<a id='nextpage' href='"+refineUrl+"?Sort="+Sort+"&pageNum=" + i + "'>");
				sb.append(" [ ");
				sb.append(i);
				sb.append(" ] ");
				sb.append("</a>");
		...
		if (end != totalPage) {
			sb.append("<a href='"+refineUrl+"?Sort="+Sort+"&pageNum=" + (end + 1) + "'>");
			sb.append("[다음]");
			sb.append("</a>");
		}
		return sb.toString();
